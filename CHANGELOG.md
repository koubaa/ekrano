...
- Goldy CPU compute backend (`GOLDY_BACKEND=cpu`, `--features goldy-cpu`): the same Slang kernels JIT on the host for debugging. Area fine (`fine_cpu.slang`) writes a packed RGBA8 buffer; [`GoldyRenderer::render_to_buffer`] and [`GoldyRenderer::render_to_pixel_sink`] withdraw it through Goldy [`PixelExchange`](https://github.com/koubaa/goldy/pull/305). Coarse stages that use `GroupMemoryBarrierWithGroupSync` do not compile on Slang's host-callable target yet. MSAA, filter-layer UAVs, images, and gradient atlas samples are skipped ([#114](https://github.com/koubaa/ekrano/issues/114)).

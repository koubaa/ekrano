...
impl BitEq for GradientCacheKey {
    fn bit_eq(&self, other: &Self) -> bool {
        self.stops.bit_eq(&other.stops) && self.interpolation_alpha_space == other.interpolation_alpha_space
    }
}
...
impl RampCache {
    ...
    pub(crate) fn add(&mut self, interpolation_alpha_space: InterpolationAlphaSpace, stops: &[ColorStop]) -> u32 {
        ...
        if let Some(entry) = self.map.get_mut(&key) {
            ...
            self.data.extend(make_ramp(stops, interpolation_alpha_space));
    ...
    (0..N_SAMPLES).map(move |i| {
        ...
        let c = if du < 1e-9 {
            ...
            match interpolation_alpha_space {
                InterpolationAlphaSpace::Premultiplied => last_c.lerp(this_c, t, HueDirection::default()),

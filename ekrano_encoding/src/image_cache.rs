...
pub struct Images<'a> {
    ...
    pub evicted: usize,
    /// True when the atlas has at least one resident entry (independent of dirty uploads).
    ///
    /// Clean frames keep residents but an empty [`Self::images`] list; atlas dims must not
    /// collapse based on dirty-list length alone.
    pub has_residents: bool,
...
impl ImageCache {
    ...
    /// True when any image is currently packed in the atlas (even if no dirty uploads).
    pub(crate) fn has_residents(&self) -> bool {
        !self.map.is_empty()
    }
    ...
    /// Clear dirty bits for residents uploaded this resolve. Call only after the GPU
    /// atlas deposits for this pass have submitted successfully.
    ...
    fn repack_to_size(&mut self, size: i32) -> bool {
        ...
        for (id, resident) in entries {
            ...
            // Growth is rare: CPU-reupload every resident instead of composing GPU
            // copies across multiple bumps in one resolve.
            resident.dirty = true;
...
mod tests {
    ...
    #[test]
    fn bump_size_dirties_residents_for_cpu_reupload() {
        let mut cache = ImageCache::new_with_sizes(16, 64);
        let image_a = image(1, 8, 8);
        ...
        assert!(cache.bump_size());
        assert!(cache.map.get(&image_a.data.id()).unwrap().dirty);
    ...
    #[test]
    fn successive_bumps_in_one_resolve_keep_final_layout_dirty() {
        let mut cache = ImageCache::new_with_sizes(16, 64);
        let image_a = image(1, 8, 8);
        ...
        assert!(cache.bump_size());
        assert!(cache.bump_size());
        assert_eq!(cache.atlas.size().width, 64);

        cache.begin_resolve();
        let xy = cache.get_or_insert(&image_a).unwrap();
        assert_eq!(cache.images.len(), 1);
        assert_eq!(cache.images[0].1, xy.0);
        assert_eq!(cache.images[0].2, xy.1);
    }

...
pub(crate) fn full_shaders(
    ...
    let mut shaders = ekrano_shaders::compile::ShaderInfo::from_default()?;
    #[cfg(not(feature = "hot_reload"))]
    let shaders = ekrano_shaders::SHADERS;
    ...
    macro_rules! add_shader {
        ...
        ($name:ident, $bindings:expr) => {
            add_shader!(
                $name,
                $bindings,
                CpuShaderType::Present(ekrano_shaders::cpu::$name)
            )
        };
    }
    ...
    let pathtag_scan = add_shader!(
        ...
        CpuShaderType::Present(ekrano_shaders::cpu::pathtag_scan)
    );
    ...
    let backdrop = add_shader!(
        ...
        CpuShaderType::Present(ekrano_shaders::cpu::backdrop)
    );

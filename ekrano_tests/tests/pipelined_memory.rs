...
/// Process-lifetime renderer.
///
/// Creating and dropping a `GoldyRenderer` per trial on DX12 WARP can TDR the
/// shared device (`Failed to create per-context DX12 fence` on the next trial).
/// Reuse one renderer for the whole test binary; trials still take the WARP
/// serial lock via [`make_device`].
fn with_shared_renderer<R>(f: impl FnOnce(&SharedTestDevice, &mut GoldyRenderer) -> R) -> R {
    let _gpu_guard = gpu_test_lock();
    let device = make_device();
    struct Slot {
        renderer: GoldyRenderer,
    }
    static SLOT: OnceLock<Mutex<Slot>> = OnceLock::new();
    let mut slot = SLOT
        .get_or_init(|| {
            let renderer = GoldyRenderer::new(&device).expect("GoldyRenderer");
            Mutex::new(Slot { renderer })
        })
        .lock()
        .unwrap_or_else(|e| e.into_inner());
    f(&device, &mut slot.renderer)
}
...
fn single_frame_ring_depth_bounded() {
    ...
    with_shared_renderer(|device, renderer| {
...
fn resource_pool_stable_under_single_frame() {
    env_logger::try_init().ok();
    with_shared_renderer(|_device, renderer| {
        let texture = test_alloc_texture(
            renderer.device(),
            WIDTH,
            HEIGHT,

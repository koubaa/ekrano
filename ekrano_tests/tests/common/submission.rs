...
//! - **Metal**: no clamp — each trial gets a fresh [`Runtime`] (see `test_device`).
...
use goldy::{Runtime, types::BackendType};

/// Clamp libtest parallelism for backends that share one process-lifetime [`Runtime`].
///
/// Metal is intentionally untouched: isolation there is per-device via
/// [`ekrano_tests::test_device`], not via thread count.
pub(crate) fn clamp_test_threads(args: &mut libtest_mimic::Arguments, device: &Runtime) {

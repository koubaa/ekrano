...
// the absolute path to the ekrano_shaders crate's manifest to be specified using the
// `UNSTABLE_BAZEL_EKRANO_SHADERS_CRATE_MANIFEST_PATH` build script environment variable.
//
// This should never be set when using cargo.
fn manifest_dir() -> PathBuf {
    use std::env;
    env::var_os("UNSTABLE_BAZEL_EKRANO_SHADERS_CRATE_MANIFEST_PATH")

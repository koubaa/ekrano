...
The "Semi-stable encoding format" section can be considered implemented. The handwritten CPU fallback stages were removed; CPU debug of the same Slang kernels is tracked in Goldy ([koubaa/goldy#292](https://github.com/koubaa/goldy/issues/292)) and wired in Ekrano via `GOLDY_BACKEND=cpu`. Area fine (`fine_cpu.slang`) writes a packed RGBA8 buffer consumed through [Goldy PixelExchange](https://github.com/koubaa/goldy/pull/305). Coarse stages that use `GroupMemoryBarrierWithGroupSync` do not compile on Slang's host-callable target yet; a full CPU frame errors until Goldy implements workgroup barriers. MSAA, images, gradient atlas samples, and filter-layer UAVs stay GPU-only.

...
The "Semi-stable encoding format" section can be considered implemented. The handwritten CPU fallback stages were removed; CPU debug of the same Slang kernels is tracked in Goldy ([koubaa/goldy#292](https://github.com/koubaa/goldy/issues/292)) and wired in Ekrano via `GOLDY_BACKEND=cpu`. Area fine writes a packed RGBA8 buffer consumed through [Goldy PixelExchange](https://github.com/koubaa/goldy/pull/305) (`render_to_buffer` / `render_to_pixel_sink`). MSAA, images, gradient atlas samples, and filter-layer UAVs stay GPU-only.
